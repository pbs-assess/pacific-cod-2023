...
The 2020 assessment for Pacific Cod (*Gadus macrocephalus*) in British Columbia (BC) [@dfo2021], found the Area 3CD stock (West Coast of Vancouver Island: WCVI; Figure \@ref(fig:fig-map)) to be in the Cautious Zone with > 99% probability.
...
The stock assessment model (see Analysis and Response) is fit to commercial and survey indices of abundance, commercial catch, and an index of mean weight, derived from commercial length samples [@forrest2020].
...
This follows a trend in reduced biological length sampling of Pacific Cod on commercial vessels that began around 2015 (Appendix A).

The RSIA therefore had two steps: 1) an evaluation to assess the potential sensitivity of the stock assessment to missing length data from the commercial fishery; and 2) an update to the status of both stocks.
...
However, given the three low index points in 2018, 2021 and 2022, DFO Science Branch has issued a RSIA requesting a stock status update for the Area 3CD stock to ensure all three of these index points are considered in a timely evaluation of stock status.
...
Fishing years are defined as beginning on April 1 for all years, and are referenced by starting year, e.g., fishing year 1957 runs from April 1, 1957 to March 31, 1958.
...
Due to lack of data, the 2020 assessment used the 2019 mean weight value for both 2019 and 2020, although this was not ideal, as the 2019 index point was also based on only two samples from the same location.
...
For 2018--2022, the model uses the 2018, 2021 and 2022 mean weight index values derived from the GLM (Figure \@ref(fig:all-mean-weight-series-3cd)).
Values for 2019 and 2020 were interpolated between 2018 and 2021 (Table \@ref(tab:tab-mean-weight-recent-3cd)).
See Appendix D for a brief discussion of model sensitivity to these choices.

## Stock assessment model

The stock assessment model is a Bayesian delay-difference model [@deriso1980], fit to survey indices, commercial CPUE, commercial catch data and and the commercial mean weight index.
...
A model-averaging approach is used to estimate stock status based on combined posterior samples from several sensitivity cases, as for the previous assessments (see Scenarios section for details). 
...
A reference model was established to represent what were considered to be the most plausible model choices across a range of assumptions.
Key characteristics of the reference model are provided in Table \@ref(tab:tab-param-table-3cd).
The reference model served as the basis from which sensitivity scenarios were developed, altering alternative model assumptions one at a time.
...
They presented a number of sensitivity analyses to treatment of the observation error parameter $\sigma_O$, one of which is included in the model-averaging set here (Sc 6b).
...
Notably, the median estimated value of natural mortality, $M$, was 0.437 *y*^-1^ (0.402--0.475 *y*^-1^) in the current assessment (Table \@ref(tab:tab-param-est-table-3cd)), intermediate between 0.42 *y*^-1^ (0.39--0.46 *y*^-1^) in 2018 [@forrest2020] and 0.45 in 2020 [@dfo2021].
...
The model was also sensitive to the assumed knife-edged age at recruitment (Sc 5a), which estimated the biomass to be lower than the other scenarios.
...
While the fishing mortality is estimated to be well-below the limit removal reference, there is a non-zero probability that the stock is below the limit reference point (LRP).

As stated in the Context of this document, a stock is considered to be below its LRP if the terminal year biomass is estimated to be below the LRP with greater than 50% probability or if there is a greater than 50% probability of being below the LRP in a one year projection with zero catch.

Table \@ref(tab:summary-tab-stock-status-avg-3cd) indicates a 23% probability (i.e., about 1 out of 4 chance) that the biomass at the beginning of 2023 is below the LRP and >99% probability that the stock is below the USR. See also Figures \@ref(fig:fig-model-average-biomass-3cd-rel) and \@ref(fig:fig-model-average-p-lrp), noting that Figure \@ref(fig:fig-model-average-p-lrp) indicates that the models estimated just over 50% probability of the stock being below the LRP in 2021.
...
1. The lack of reliable age composition data for this species, which would provide additional information about recruitment strength, $M$, and gear selectivity. Reliable age composition data would allow for better estimates of age at recruitment to the fishery, to which the scale of biomass estimates is sensitive, and which is very uncertain. Rescaling the biomass would not necessarily influence estimates of stock status, which are based on historical reference points that rescale with biomass. However, estimates of fishing mortality would be affected;
...
Model-averaged estimates of biomass indicated a 23% probability that the stock is below the LRP at the beginning of 2023, and a 16% probability that the stock would be below the LRP at the beginning of 2024 under zero catches.
...
We also note uncertainties in the scale of the model-estimated biomass, given uncertainties in $M$ and age at recruitment (see above section).
...
There has not been a thorough analysis of potential causes for the current decline in the Area 3CD Pacific Cod stock, which may be related to climate change, poor egg hatching survival, declines in body condition, fish movement, fishing, or other factors.
...
Alternative approaches may include length-based methods [e.g., @haist1995; @haist1998], or more data-limited approaches, such as adjusting annual TACs based on changes in the survey indices.
