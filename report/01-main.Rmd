...
The assessment found the Area 3CD stock (West Coast of Vancouver Island: WCVI; Figure \@ref(fig:fig-map)) to be in the Cautious Zone with > 99% probability [@dfo2021].
...
Given the low index point in 2018, the 2020 Science Response recommended updating the assessment following the 2020 WCVI survey.
However, due to the COVID-19 pandemic, the scheduled 2020 WCVI synoptic bottom trawl survey was postponed to 2021, returning to its regular biennial schedule in 2022.
Both the 2021 and 2022 index points were of a similar low magnitude to the 2018 observation (Figure \@ref(fig:fig-surv-canadian)).
Catches from 2018--2020 were also lower than in previous years (Table \@ref(tab:tab-catch-3cd), Figure \@ref(fig:fig-catch-3cd)).
The commercial CPUE index also decreased substantially in 2018 but has increased slightly since then, with increases in commercial catch (Figure \@ref(fig:fig-summary-cpue-indices-3cd)).

The stock assessment is a Bayesian delay-difference model [@deriso1980], fit to commercial and survey indices of abundance, commercial catch, and an index of commercial mean weight [@forrest2020]. The commercial mean weight index is derived from commercial length samples, and provides information about scale and recruitment strength to the model.
...
However, even before the pandemic, there was a notable reduction in biological sampling of Pacific Cod on commercial vessels, starting around 2015 (Appendix A).
...
The request had two steps: 1) an evaluation to assess the potential sensitivity of the stock assessment to missing length data from the commercial fishery; and 2) an update to the status of both stocks.
...
However, given the three low index points in 2018, 2021 and 2022, DFO Science Branch has issued a RSIA requesting a stock status update to ensure all three of these index points are considered in a timely evaluation of stock status.
While the last assessment found that there was low probability (2%--10%) of the stock being below its limit reference point (LRP) in 2022 under all catch scenarios [@dfo2021], the stock has failed to rebound under low catch rates, and catches have increased since 2020 (Table \@ref(tab:tab-catch-3cd)).
...
However, due to the ongoing absence of commercial length samples, this update uses the GLM developed in 2022 to update the last five years of the annual commercial mean weight index, based on values from the survey mean weight index.
...
This assessment provides a stock status update for the 3CD stock only. 

## Description of the fishery {#fishery}

Pacific Cod in BC are caught almost entirely in the groundfish bottom trawl fishery, which is part of BC's integrated groundfish fishery [@dfo2023], mainly in 50–200 m depth zones [@forrest2020].
Most catch taken in Area 5CD [@forrest2020; Figure\@ref(fig:fig-map)], with smaller catches from Areas 5AB and 3CD, with near negligible catch taken from the west coast of Haida Gwaii, Area 5E (< 0.5% of total average annual catch since 1985).
...
Data from fishery-independent and commercial fishery sources were available up until March 31, 2023.
...
### Canadian bottom trawl surveys {#surveys}

Survey indices (Table \@ref(tab:surv-canadian-table) and Figure \@ref(fig:fig-surv-canadian)) were calculated using a swept area analysis, documented in Appendix A of @forrest2020.
...
For comparison, survey indices in Areas 5AB and 5CD are also shown in (Table \@ref(tab:surv-canadian-table) and Figure \@ref(fig:fig-surv-canadian)).
...
### Commercial CPUE {#cpue}

Standardized commercial CPUE indices were developed for the historical (1956--1995) and modern (1996--2019) periods (Figures  \@ref(fig:fig-summary-cpue-indices-3cd) and \@ref(fig:fig-summary-cpue-indices-3cd)).
...
The historical-period GLMMs included predictors for depth, locality, month, and a locality-year interaction (Figure \@ref(fig:fig-summary-cpue-indices-3cd)).
The modern-period GLMMs included predictors for depth, latitude, locality, month, vessel, and a locality-year interaction (Figure \@ref(fig:fig-summary-cpue-indices-3cd)).

### Annual mean weight in commercial catch {#mean-weight-index}
...
There has been no biological sampling on commercial trawl vessels since 2019, and a decline in commercial sampling of Pacific Cod, starting around 2015 (Appendix A).
...
In the absence of representative biological samples for the last years of the time series, a methodology was developed for predicting the commercial mean weight index from a mean weight index derived from length samples from WCVI synoptic survey  (Appendices B and C).

The GLM predicts values for the commercial mean weight index only for years when there was a survey (i.e., predicted index values were generated for even-numbered years from 2004 to 2018, then 2021 and 2022; Figure \@ref(fig:all-mean-weight-series-3cd)).
A decision had to be made, therefore, about how to include the GLM-derived index values in the commercial mean weight index, since there were no GLM-derived values for 2017, 2019 or 2020.

For consistency with previous assessments [@forrest2020; @dfo2021], the model uses the commercial mean weight index derived directly from commercial length samples for the years 1956--2016.
...
See Appendix D for a brief discussion of model sensitivities to these choices.

**TODO: MOVE next para to an appendix**

The TWG explored a number of sensitivity analyses, which tested model sensitivity to alternative approaches to updating the commercial mean weight index, including indices that excluded the interpolated values, did not include the GLM values, or variously included or excluded the 2017 and 2019 index points from the commercial data.
The model was largely insensitive to these alternative treatments, mainly because the GLM-derived index values were consistent with the other data sources in the model (see Reference Model Results), resulting in poor fits to alternative versions of the index and, therefore, little impact on recruitment and biomass estimates.

## Stock assessment model {#assessment}

The current stock assessment model is a Bayesian delay-difference model [@deriso1980], fit to survey indices, commercial CPUE, commercial catch data and commercial mean weights.
...
A model-averaging approach is used to estimate stock status based on combined posterior samples from several sensitivity cases, as for the previous assessments [@forrest2020; @dfo2021]. 
This is to address some of the key irresolvable uncertainties associated with the stock assessment.
Major axes of uncertainty are catchability ($q$) in the indices of abundance, the magnitude of natural mortality ($M$), age at recruitment to the fishery, and the magnitude of observation errors in the surveys and mean weight [@forrest2020].
See Scenarios section for details.
...
2. The LRP is the lowest estimated biomass agreed upon as an undesirable state to be avoided. For Area 3CD it is the estimated biomass in 1986.
...
Key characteristics of the model are provided in Table \@ref(tab:tab-param-table-3cd) [see @forrest2020 for details].
...
The set of model scenarios was agreed upon at the Canadian Science Advisory Secretariat (CSAS) Regional Peer Review (RPR) meeting for model averaging in the 2018 assessment [@dfo2019].
The same set was used again in the 2020 stock assessment update [@dfo2021] and is used again here (updated with catch and index data to the end of the 2022 fishing year, with updates to the annual mean weight index described above).
...
## Reference model results {#ref-results}

Results are presented from the reference model, to demonstrate model performance and to provide comparison with the 2020 stock assessment update.
Stock status, presented in the next section, is based on combined, averaged posterior model results.

The joint posterior distribution for each model was numerically approximated using the Markov Chain Monte Carlo (MCMC) routines built into AD Model Builder (Metropolis-Hastings algorithm) [@fournier2012]. 
...
Notably, the median estimated value of natural mortality, $M$, was 0.437 *y*^-1^ (0.402--0.475 *y*^-1^) in the current assessment (Table \@ref(tab:tab-param-est-table-3cd)), compared to 0.42 *y*^-1^ (0.39--0.46 *y*^-1^) in 2018 [@forrest2020] and 0.45 in 2020 [@dfo2021].
...
As in previous assessments, of the seven scenarios, the model was most sensitive to the assumed standard deviation in the prior for survey catchability ($q$) (Sc 2d).
The model was also sensitive to the assumed knife-edged age at recruitment (Sc 5a), which estimated the biomass to be much lower than the other scenarios.
...
1. The lack of reliable age composition data for this species, which would provide additional information about recruitment strength, $M$, and gear selectivity. Reliable age composition data would allow for better estimates of age at recruitment to the fishery, to which the biomass estimates are sensitive;
...
We also note uncertainties in the scale of the model-estimated biomass, given uncertainties in $M$ and age at recruitment.
...
This may also be advantageous, given the uncertainties inherent in this model, particularly uncertainty in $M$, catchability, and age at recruitment.
Alternative approaches may include length-based methods [e.g., @haist1995; @haist1998], or even more data-limited approaches, such as adjusting annual TACs based on changes in the survey indices.
...
Exploration of models that allow for time-varying productivity [e.g., @mildenberger2019] may also be informative.
