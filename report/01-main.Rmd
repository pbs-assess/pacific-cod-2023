...
However, due to the ongoing absence of commercial length samples, this update uses the GLM developed in 2022, which predicts commercial mean weight index values from a survey mean weight index, to update the last five years of the annual commercial mean weight index.
The survey mean weight index and the GLM are described in Appendices B and C.

Prior to estimating model-averaged stock status, the impacts of different treatments of the commercial annual mean weight index are evaluated, including two scenarios that use recent index values derived from the GLM presented to the TWG in 2022. 
The commercial annual mean weight index from one of these scenarios is selected and used in the seven models used for model-averaging. 
