...
The 2020 assessment for Pacific Cod (*Gadus macrocephalus*) in British Columbia (BC) found the Area 3CD stock (West Coast of Vancouver Island: WCVI; Figure \@ref(fig:fig-map)) to be in the Cautious Zone with > 99% probability [@dfo2021].
...
The commercial CPUE index also decreased substantially in 2018 but has increased since then, following increases in commercial catch (Figure \@ref(fig:fig-summary-cpue-indices-3cd)).
...
The 2022 RSIA therefore had two steps: 1) assessment of the potential sensitivity of the stock assessment to missing length data from the commercial fishery; and 2) a stock status update and catch advice for both stocks.
...
Because of this problem with the 5ABCD stock and limited staff resources, Step 2 did not proceed.
...
While the last assessment found that there was low probability (2%--10%) of the stock being below its limit reference point (LRP) in 2022 under all catch scenarios [@dfo2021], the stock has failed to rebound under low catch rates, and catches have increased (Table \@ref(tab:tab-catch-3cd)).
...
Due to lack of data, the 2020 assessment used the 2019 mean weight value for both 2019 and 2020, although this was not ideal, as the 2019 index point was based on only two samples from the same location.

In the absence of representative biological samples for the last years of the time series, a GLM was developed to predict the commercial mean weight index from a mean weight index derived from length samples from WCVI synoptic survey (Appendices B and C).
Model sensitivities are shown in Appendix D.

The GLM predicts values for the commercial mean weight index only for years when there was a survey (i.e., predicted index values were generated for even-numbered years from 2004 to 2018, then 2021 and 2022; Appendix C: Figure \@ref(fig:all-mean-weight-series-3cd)).
Therefore, a decision had to be made about how to include the GLM-derived index values in the commercial mean weight index, since there were no GLM-derived values for 2017, 2019 or 2020.

For consistency with previous assessments, the model uses the commercial mean weight index derived directly from commercial length samples for the years 1956--2016.
...
See also @forrest2018 for evaluation of historical reference points for the 5CD stock of Pacific Cod.

## Model scenarios

A model-averaging approach is used to estimate stock status based on combined posterior samples from several sensitivity cases, as in the two previous assessments [@forrest2020; @dfo2019]. 
...
The set of model scenarios for model-averaging was agreed upon at the Canadian Science Advisory Secretariat (CSAS) Regional Peer Review (RPR) meeting in 2018 [@dfo2019].
...
The model was also sensitive to the assumed knife-edged age at recruitment (Sc 5a), which estimated the biomass to be lower than the other scenarios when recruitment was assumed to occur at age 3.
...
There are also uncertainties in the scale of the model-estimated biomass, given uncertainties in $M$ and age at recruitment.
The model was especially sensitive to assumptions about the age at recruitment, with lower biomass estimates when recruitment age was assumed to be 2 *y* instead of 3 *y*.
Reducing the estimated scale of the biomass would not necessarily influence estimates of stock status, which are based on historical reference points that rescale with biomass.
However, estimates of fishing mortality would increase.
