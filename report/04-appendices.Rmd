...
The commercial mean weight index, calculated using the methods in Appendix C of @forrest2020 is shown in Figure \@ref(fig:fig-comm-mean-weights-3cd), which also indicates the two index values that were removed due to low sampling effort (2017 and 2019, see Appendix A).

Figure \@ref(fig:fig-meas-vs-calc-weights-3cd) shows the relationship between directly observed weights (from weighing individual fish) vs weights calculated (obtained from Equation 1).
...
```{r fig-comm-vs-survey-weights-3cd, fig.cap="Comparison of cmercial mean weight index (line) and survey mean weight index (points). Area of the circles indicates the number of sampling events. To aid visualization, the commercial mean weight index is truncated to begin in 2000.", out.width="5in"}
