...
The decision to exclude the 2017 index point was agreed in the 2018 CSAS meeting [@forrest2020] and carried through to the 2020 update [@dfo2021].
...
Given the obvious problems with basing index points on such limited samples, we don't think it is justified to include the 2017 data point or change model parameters just to fit to it.

However, we have run some scenarios to illustrate the effects of including the 2017 index point and changing model settings to fit to it.
The first is a scenario based on Scenario 1 of the TWG Report, which includes the 2017 mean weight index value but makes no changes to model parameters.
Comparisons between the model fit to the annual mean weight index in Scenario 1 (without 2017) and Scenario 1a (with 2017) are shown in Figures \@ref(fig:fig-base-mean-weight-3cd) and \@ref(fig:fig-mean-weight-3cd-2017).
The model does not fit the 2017 value, similarly missing it and other very high mean weight index values.

As noted in previous Pacific Cod assessments, the model in general does not fit the mean weight index very well, especially prior to 1996, where smaller fish are underrepresented in the data due to the absence of at-sea observers when only "keepers" were represented in the database [@forrest2015; @forrest2020].
The only available mechanism to directly adjust the fit to the annual mean weight data in this model is the standard deviation parameter $\sigma_W$, which controls the closeness of the fit to the annual mean weight index.
...
For illustration, we present two model scenarios, based on Scenario 1a, where we reduced $\sigma_W$ from 0.2 to 0.15 and 0.1, respectively (Figures \@ref(fig:fig-mean-weight-3cd-sigw-015) and \@ref(fig:fig-mean-weight-3cd-sigw-01)).
Reducing $\sigma_W$ had the effect of slightly improving the model fit to higher values of the annual mean weight index, including 2017, and some pre-1996 values, which are known to be biased. 
This in turn led to a noticeable scaling up of the recruitment and biomass estimates, throughout the time series, including recent years, especially for the scenario with $\sigma_W = 0.1$ (Figures \@ref(fig:fig-sens-recr-3cd-sigw) and \@ref(fig:fig-sens-biomass-3cd-sigw)).
...
The annual mean weight index has always been a problematic component of this data-limited assessment, which is why the authors have settled on $\sigma_W = 0.2$, to balance this trade-off between fitting recent points well while allowing some underestimation of the pre-1996 index.
Estimating the scale of this model has also been problematic in the past, where the model is very sensitive to several parameters including $\sigma_W$, the index observation error term and priors for catchability and natural mortality.
...
We underline that the 2023 Science Response will include six other scenarios in the model-averaged set, which includes a scenario with $\sigma_W = 0.15$.
...
```{r fig-mean-weight-3cd-2017, fig.cap="Scenario 1 model MPD fit to the annual mean weight index, which includes a value for 2017."}
...
```{r fig-mean-weight-3cd-sigw-015, fig.cap="Scenario 1 model MPD fit to the annual mean weight index, with $\\sigma_W = 0.15$."}
...
```{r fig-mean-weight-3cd-sigw-01, fig.cap="Scenario 1 model MPD fit to the annual mean weight index, with $\\sigma_W = 0.1$."}
...
```{r fig-sens-recr-3cd-sigw, fig.cap="Sensitivity of recruitment estimates to the value of $\\sigma_W$. Points shows the posterior medians and the bars represent the 95\\% credible interval. The lower panel is zoomed in to 2015--2022.", fig.asp=.9}
g1 <- r.plot(c(base.model.3cd, sens.models.44), c(base.model.3cd.name, sens.models.name.44))
g2 <- r.plot(c(base.model.3cd, sens.models.44), c(base.model.3cd.name, sens.models.name.44)) +
  coord_cartesian(xlim = c(2015, 2022), ylim = c(0, 7000)) +
  scale_x_continuous(breaks = 2015:2022)
cowplot::plot_grid(g1, g2, ncol = 1, align = "v")
```

```{r fig-sens-biomass-3cd-sigw, fig.cap="Sensitivity of biomass estimates to the value of $\\sigma_W$. Thick solid lines shows the posterior medians and the grey shaded regions represent the 95\\% credible interval. The lower panel is zoomed in to 2015--2022.", fig.asp=.9}
g1 <- b.plot(c(base.model.3cd, sens.models.44), c(base.model.3cd.name, sens.models.name.44))
g2 <- b.plot(c(base.model.3cd, sens.models.44), c(base.model.3cd.name, sens.models.name.44)) +
...
As illustrated in Section 1, the model is actually very sensitive to the annual mean weight index.
...
However, of most relevance to this update, the GLM presented in the TWG Report showed a very strong relationship between the commercial and survey annual mean weight indices.
...
Consequently, the model fits the last five years of the annual mean weight index very well, which justifies their inclusion in this update (Figure \@ref(fig:fig-base-mean-weight-3cd)).
