...
We are not sure what is meant by "the uncertainty in both sets of calculations
of mean weight should be included in the transformation". If we are talking
about propagating uncertainty from the length-weight relationships to the
assessment model: (1) the length-weight relationship is one of the tightest
relationships in fisheries science and so propagating its uncertainty is
unlikely to make a meaningful difference (or at least is not the first place we
would consider propagating uncertainty), (2) for the commercial samples, the
established model does not propagate this uncertainty (nor do any other BC
groundfish assessment models to our knowledge) and this update is the not
appropriate time to add this to the model, and (3) for the survey prediction of
commercial mean weights, we note in response 2e that we did check this and it
made no meaningful difference to the assessment outputs.
...
The model is a generalized linear model fit with maximum likelihood with
a Gamma observation distribution and a log link. We do not think it is
reasonable to include additional covariates in an attempt to increase
explanatory power given the limited number of data points---there are seven
data points and two estimated fixed effects, resulting in five residual degrees
of freedom. We could certainly improve the fit to these data, but that would be
highly unlikely to improve out-of-sample prediction, which is the purpose of
the model.

We disagree that the model "failed quite badly in both 2012 and 2016". The
statement that the "observed values lie well outside of the model credibility
envelope" reflects a misunderstanding of these confidence intervals. These
confidence intervals (and most reported confidence intervals) represent
uncertainty around the mean expectation---not around new observations. In R's
`predict.glm()`, a confidence interval around new observations would be called
a prediction interval or in Bayesian statistics it would be called a posterior
predictive interval. The QQ plot indicates that the observations are in fact
consistent with the model given the observation likelihood and we may include
a predictive interval within the Science Response to emphasize this.  Although
not shown in the last TWG Report, we did explore a Bayesian version of the GLM
where we simulated new observations repeatedly and fit the base assessment
model to them. Even with extreme draws from the joint posterior, the base
assessment model was not meaningfully changed.
...
However, we note that the potential scale reduction statistics ($\hat{R} \le 101$ and $n_\mathrm{eff} > 190$) [@gelman1992; @rstan2018] were consistent with convergence (TWG Report, Table 9).
