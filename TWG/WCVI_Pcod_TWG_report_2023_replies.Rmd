...
```{r fig-meas-vs-calc-index-compare-2, fig.cap="Commercial mean weight index and survey mean weight index. Area of the circles indicates the number of sampling events. To aid visualization, the commercial mean weight index is truncated to begin in 2000.", out.width="4in", echo = FALSE}
...
However, we note that the the potential scale reduction statistic ($\hat{R}$) [@gelman1992; @rstan2018] was consistent with convergence  (TWG Report, Table 9).
...
We recommend proceeding with the 2023 Science Response based on Scenario 1, with the following updates:

1. Update the length-weight parameters in all scenarios;

2. Continue to use the calculated weights from lengths, given very little difference in the index to that build using the observed weights;

3. Continue to exclude the 2017 annual mean weight index point;

4. Use an MCMC chain length of 10 million in all models, as for previous assessments; and

5. Update plots and correct typographical errors as suggested.
...
Given the poor predictive power of the GLM for predicting a commercial mean weight index for the Area 5ABCD population (unpublished 2022 TWG Report), there is already no clear path forward for that stock.
...
We would recommend peer-reviewing any new model separately from the provision of advice (e.g., a "Benchmark" process similar to that followed by ICES; or a "Framework" process followed by DFO in the Maritimes region).
