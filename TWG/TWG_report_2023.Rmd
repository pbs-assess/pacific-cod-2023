...
Analyses found that the 2020 Area 3CD model was sensitive to the removal of the last three years of the mean weight index (unpublished report, available on request).
...
Prior to estimating model-averaged stock status, the impacts of different treatments of the commercial mean weight index are evaluated, including two scenarios that use recent index values derived from the GLM presented to the TWG in 2022. 
The commercial mean weight index from one of these scenarios is selected and used in the seven models used for model-averaging. 
...
The methodology was presented to the TWG in 2022, who found it provided a reasonable fit to the commercial mean weight index derived from commercial data (see also Appendix C).
...
Step 1. Develop am annual mean weight index based on length samples from the WCVI Synoptic Survey (see Appendix B for methods and results).

Step 2. Use a GLM with gamma observation error and a log link to predict commercial annual mean weight index values from the survey mean weight index (see Appendix C for methods and results).
...
As in previous assessments, all scenarios (except one sensitivity scenario) assumed an observation error of $\sigma_W$ = 0.2, which influences the fit to the mean weight in the likelihood function [@dfo2021].
...
Table \@ref(tab:summary-tab-stock-status-avg-3cd) indicates a 24% probability that the biomass at the beginning of 2023 is below the LRP and >99% probability that the stock is below the USR. See also Figures \@ref(fig:fig-model-average-biomass-3cd-rel) and \@ref(fig:fig-model-average-p-lrp), noting that Figure \@ref(fig:fig-model-average-p-lrp) indicates that the model set estimated just over 50% probability of the stock being below the LRP in 2021.
...
The survey annual mean weight index was a reasonable predictor of the commercial annual mean weight index in Area 3CD.
...
There has not been a thorough analysis of potential causes for the current decline in the Area 3CD Pacific Cod stock, which may be related to climate change, declines in body condition, movement, fishing, or other factors.
