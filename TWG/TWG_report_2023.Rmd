...
Given the ongoing low catches of Area 3CD Pacific Cod and model estimates of low fishing mortality [@dfo2021], the Pacific Groundfish Management Unit did not request updated catch advice for 2023.
...
Step 2. Use GLM with gamma observation error and a log link to predict commercial mean weight index values from the survey mean weight index (see Appendix C for methods and results).
...
As in previous assessments, all scenarios (except one sensitivity scenario) assumed an observation error of $\sigma_W$ = 0.2, which controls the fit to the mean weight in the likelihood function [@dfo2021].
...
Model-averaged estimates of biomass indicated a 24% probability that the stock is below the LRP and a 16% probability that the stock would be below the LRP at the beginning of 2024 under a zero catch policy.
...
```{r fig-sens-recr-3cd-zoomed, fig.cap="Sensitivity of recruitment estimates to commercial mean weight scenarios. Points shows the posterior medians and the bars represent the 95\\% credible interval. See Methods for scenario descriptions. \\textbf{This version is zoomed in to 2015--2022.}"}
r.plot(c(base.model.3cd, sens.models.11), c(base.model.3cd.name, sens.models.name.11)) +
...
```{r fig-sens-biomass-3cd-zoomed, fig.cap="Sensitivity of biomass estimates to commercial mean weight scenarios. Thick solid lines shows the posterior medians and the grey shaded regions represent the 95\\% credible interval. See Methods for scenario descriptions. \\textbf{This version is zoomed in to 2015--2022.}"}
b.plot(c(base.model.3cd, sens.models.11), c(base.model.3cd.name, sens.models.name.11)) +
...
We used a GLM with a gamma observation model and a log link, to estimate commercial mean weights ($y_t$) in year $t$ based on survey mean weights ($W_t$), calculated in Appendix B:
...
The GLM estimated an intercept ($\beta_0$) of 0.34 (95% CI: XX--XX), or 1.41 kg (95% CI: XX--XX) in natural space. The model estimated $\beta_1$ as 0.75 (95% CI: XX--XX) meaning a 1% increase in survey mean weight was associated with a 0.75% increase in commercial mean weight.

A quantile-quantile plot of residuals transformed to have a N(0, 1) distribution if the model were consistent with the data revealed... TODO finish this and plots... deviance explained... leverage...
