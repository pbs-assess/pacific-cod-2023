...
```{r fig-model-average-biomass-3cd-rel, fig.cap="Combined posterior biomass over biomass at the LRP and biomass at the USR for the model-averaged set. Thick solid line shows the posterior median and the grey shaded regions represents the 50\\% and 95\\% credible intervals. Horizontal dashed line represents a ratio of 1.", fig.asp=1}
...
cowplot::plot_grid(g, g1, ncol = 1)
```

```{r fig-model-average-p-lrp, fig.cap="Probability of biomass being below the LRP from 2014 to 2023. The horizontal dashed line shows eight threshold of 0.5 probability."}
x |> 
  group_by(year) |> 
...
We used a GLM with a gamma observation model and a log link [@mccullagh1989, p. 292], to estimate commercial mean weights ($y_t$) in year $t$ based on survey mean weights ($W_t$), calculated in Appendix B. Our model was fit as
\begin{align}
...
and the term $\mu_t / \phi$ represents the gamma scale parameter.
